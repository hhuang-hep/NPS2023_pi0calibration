...
# useful commend to resolve problem jobs:
#swif2 modify-jobs -workflow <workflow name> -ram set <X>gb -problems SLURM_OUT_OF_MEMORY
...
if [ ${2:-unset} == "add" ]; then
    ...
    if [ -d "$DIR" ]; then
        echo "$remote_output_1_Dir exists. Files inside might be overwritten."
    
    else
        echo "$remote_output_1_Dir doesn't exist. Create the directory for output files."
        mkdir $remote_output_1_Dir
    fi
...
while read -r run nseg; do
    for ((iseg=0; iseg<nseg; iseg++)); do
    # if (( (run == 4745 && iseg == 0) || (run == 4803 && iseg == 1) )); then
        key="${run},${iseg}"
        $ECHO \
        swif2 add-job \
            -create \
